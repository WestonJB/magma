...
/***************************************************************************//**
    ...
    ZSYTRF_nopiv computes the LDLt factorization of a complex symmetric
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    nb = magma_get_zsytrf_nopiv_nb(n);
    ...
    if (upper) {
        ...
        for (j=0; j < n; j += nb) {
            ...
            magma_zsytrf_nopiv_cpu( MagmaUpper, jb, ib, A(j, j), lda, info );
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
        ...
        for (j=0; j < n; j += nb) {
            ...
            magma_queue_sync(queues[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            magma_zsytrf_nopiv_cpu( MagmaLower, jb, ib, A(j, j), lda, info );
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
