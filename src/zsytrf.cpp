...
/***************************************************************************//**
    ...
    ZSYTRF computes the factorization of a real symmetric matrix A
    ...
    Further Details
    ===============
    If UPLO = MagmaUpper, then A = U*D*U^T, where
    U = P(n)*U(n)* ... *P(k)U(k)* ...,
    ...
    If UPLO = MagmaLower, then A = L*D*L^T, where
       L = P(1)*L(1)* ... *P(k)*L(k)* ...,
    ...
    magma_int_t *ipiv, magma_int_t *info)
{
    ...
    /* .. Local Scalars .. */
    magma_int_t nb = magma_get_zsytrf_nb(n);
    ...
    if ( upper ) {
        ...
        for (k = n-1; k >= 0; k -= kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_zlasyf_gpu( MagmaUpper, nk, kb, &kb, dA( 0, 0 ), ldda,
                ...
                lapackf77_zsytf2( MagmaUpperStr, &nk, A( 0, 0 ), &lda, &ipiv[0], &iinfo );
                kb = k+1;
            }
        ...
        for (k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_zlasyf_gpu( MagmaLower, nk, nb, &kb, dA( k, k ), ldda,
                ...
                lapackf77_zsytf2( MagmaLowerStr, &nk, A( k, k ), &lda, &ipiv[k], &iinfo );
            }
            /* Set INFO on the first occurrence of a zero pivot */
